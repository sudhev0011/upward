...
export class ProviderServiceController {
  constructor(
    ...
    private readonly _setProviderServicePriceUseCase: IConfigureProviderServiceUseCase,
    private readonly _deleteProviderServiceUseCase: IDeleteProviderServiceUseCase,
  ) {}

  createProviderService = async (
    ...
    const bodySchema = CreateProviderServiceRequestDtoSchema.omit({
      providerId: true,
    });
    ...
    try {
      const result = await this._createProviderServiceUseCase.execute({
        providerId: userId,
        serviceId: parsed.data.serviceId,
      });
      sendSuccessResponse(
        res,
        successResponse.CREATE_PROVIDER_SERVICE_SUCCESS,
        result,
      );
    } catch (error) {
  ...
  getProviderServicesByCategory = async (
    req: AuthenticatedRequest,
    res: Response,
    next: NextFunction,
  ) => {
    ...
    try {
      const result = await this._getProviderServicesByCategoryUseCase.execute({
        providerId,
        query: parsed.data,
      });
      sendSuccessResponse(
        res,
        successResponse.PROVIDER_SERVICE_FETCH_SUCCESS,
        result,
      );
    } catch (error) {
      handleAsyncError(error, next);
  ...
  setProviderServicePrice = async (
    ...
    try {
      const result = await this._setProviderServicePriceUseCase.execute(
        parsed.data,
      );
      sendSuccessResponse(
        res,
        successResponse.SET_PROVIDER_SERVICE_PRICE_SUCCES,
        result,
      );
    } catch (error) {
      handleAsyncError(error, next);
  ...
  deleteProviderService = async (
    ...
    try {
      ...
      sendSuccessResponse(
        res,
        successResponse.DELETE_PROVIDER_SERVICE_SUCCESS,
        null,
      );
    } catch (error) {
      handleAsyncError(error, next);

...
const socialLinkSchema = z.object({
  ...
  link: z.union([z.string().url('Please enter a valid URL'), z.literal('')]).optional(),
});

export const providerProfileSchema = z.object({
  name: z.string().trim().min(3, 'Name is required').max(100, 'Name must not exceed 100 characters').optional(),
  ...
  experience: z.string().regex(/^\d+$/, "Must be a positive number").optional(),

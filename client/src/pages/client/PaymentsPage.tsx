...
const PaymentsPage = () => {
  ...
  const totalSpent = TRANSACTIONS.filter((t) => t.type === "debit").reduce(
    (s, t) => s + Math.abs(t.amount),
    0,
  );
  const totalRefund = TRANSACTIONS.filter((t) => t.type === "credit").reduce(
    (s, t) => s + t.amount,
    0,
  );
        ...
        <div className="rounded-2xl border border-gray-100 bg-white shadow-sm overflow-hidden">
          ...
          <div className="divide-y divide-gray-50">
            {visible.map((t) => (
              ...
              >
                <div
                  className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl ${t.type === "credit" ? "bg-blue-50 text-blue-500" : "bg-gray-100 text-gray-500"}`}
                >
                  {t.type === "credit" ? (
                ...
                <div className="flex items-center gap-3 shrink-0">
                  <span
                    className={`rounded-full border px-2.5 py-0.5 text-xs font-semibold capitalize ${STATUS_STYLES[t.status]}`}
                  >
                    {t.status}
                  </span>
                  <p
                    className={`text-sm font-extrabold w-20 text-right ${t.type === "credit" ? "text-blue-500" : "text-gray-900"}`}
                  >
                    {t.type === "credit" ? "+" : ""}$
      ...
      {tab === "methods" && (
        <div className="flex flex-col gap-4">
          {CARDS.map((c) => (
            <div
              key={c.id}
              className={`rounded-2xl border bg-white p-5 shadow-sm flex items-center gap-4 ${c.primary ? "border-[#719FC4]/40" : "border-gray-100"}`}
            >
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  {c.primary && (
                    <span className="rounded-full bg-[#EAF2F9] px-2 py-0.5 text-[10px] font-bold text-[#5585A8]">
                      Primary
                    </span>
                  )}
                </div>
              ...
              <div className="flex gap-2">
                {!c.primary && (
                  <button className="rounded-lg border border-gray-200 px-3 py-1.5 text-xs font-semibold text-gray-500 hover:bg-gray-50 transition-colors">
                    Set Primary
                  </button>
                )}
                <button className="rounded-lg border border-red-200 px-3 py-1.5 text-xs font-semibold text-red-400 hover:bg-red-50 transition-colors">
                  Remove
                </button>
              </div>
            </div>

  ...
  return (
    <div className="p-4 md:p-6 max-w-[1400px] mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-extrabold tracking-tight text-gray-900">My Bookings</h1>
          ...
          {FILTERS.map((f) => (
            <button
              ...
              className={`rounded-lg px-3 py-1.5 text-xs font-semibold capitalize transition-all ${
                filter === f ? "bg-[#719FC4] text-white shadow-sm" : "bg-gray-100 text-gray-500 hover:bg-gray-200"
              }`}
            >
              {f}
            </button>
      ...
      <div className={`grid gap-5 ${selected ? "lg:grid-cols-3" : "grid-cols-1"}`}>
        {/* Card list */}
        <div className={`flex flex-col gap-3 ${selected ? "lg:col-span-2" : ""}`}>
          {visible.length === 0 && (
          ...
          {visible.map((b) => (
            <div
              ...
              className={`cursor-pointer rounded-2xl border bg-white p-5 transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 ${
                selected === b.id ? "border-[#719FC4]/40 shadow-md" : "border-gray-100"
              }`}
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-start gap-3 min-w-0">
                  <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-[#719FC4] text-xs font-bold text-white">
                    {b.providerInitials}
                  </div>
                  <div className="min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-2 flex-wrap">
                      <span className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${CATEGORY_STYLES[b.category]}`}>{b.category}</span>
                      <span className={`rounded-full border px-2.5 py-0.5 text-xs font-semibold capitalize ${STATUS_STYLES[b.status]}`}>{b.status}</span>
                      {b.rating && (
                ...
                <div className="text-right flex-shrink-0">
                  <p className="text-lg font-extrabold tracking-tight text-gray-900">${b.price.toLocaleString()}</p>
          ...
          <div className="rounded-2xl border border-gray-100 bg-white shadow-sm p-6 h-fit sticky top-6">
            ...
            <div className="flex items-center gap-3 mb-5 pb-5 border-b border-gray-100">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400">{detail.category} Service</p>
              </div>
            </div>
            {[
            ...
            <div className="mt-5 flex flex-col gap-2">
              <button className="w-full rounded-xl bg-[#719FC4] hover:bg-[#5585A8] py-2.5 text-sm font-bold text-white transition-all">Message Provider</button>
              {detail.status === "active" && (
                <button className="w-full rounded-xl border border-red-200 text-red-500 hover:bg-red-50 py-2.5 text-sm font-semibold transition-all">Cancel Booking</button>
              )}
              {detail.status === "completed" && !detail.rating && (
                <button className="w-full rounded-xl border border-amber-200 text-amber-600 hover:bg-amber-50 py-2.5 text-sm font-semibold transition-all">Leave a Review</button>
              )}
            </div>
          </div>
        )}

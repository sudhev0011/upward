...
const PENDING_REVIEWS = [
  { id: "pr1", title: "Electrical Inspection", provider: "Tom R.",  initials: "TR", date: "Feb 20, 2026", price: 95  },
  { id: "pr2", title: "Interior Painting",     provider: "Mike B.", initials: "MB", date: "Jan 8, 2026",  price: 340 },
];

const SUBMITTED_REVIEWS = [
  { id: "sr1", title: "Logo & Brand Design",  provider: "Priya K.", initials: "PK", rating: 5, date: "Mar 1, 2026",  comment: "Absolutely stunning work. Priya captured our brand perfectly and delivered ahead of schedule. Highly recommended!" },
  { id: "sr2", title: "Home Deep Cleaning",   provider: "Sarah M.", initials: "SM", rating: 4, date: "Feb 5, 2026",  comment: "Very thorough and professional. The apartment was spotless. Minor issue with timing but otherwise great." },
  { id: "sr3", title: "AC Maintenance",       provider: "CoolFix",  initials: "CF", rating: 5, date: "Jan 20, 2026", comment: "Fast, efficient and friendly. Fixed the issue on the first visit. Will definitely book again." },
];
  ...
  <div className="flex items-center gap-1">
    {[1,2,3,4,5].map((n) => (
      <button key={n} onClick={() => onChange(n)} className="transition-transform hover:scale-110">
        <Star className={`h-7 w-7 ${n <= value ? "fill-amber-400 text-amber-400" : "text-gray-300"}`} />
      </button>
    ))}
  </div>
);
...
const ReviewsPage = () => {
  ...
  return (
    <div className="p-4 md:p-6 max-w-[1400px] mx-auto">
      <div className="mb-6">
        <h1 className="text-2xl font-extrabold tracking-tight text-gray-900">Reviews</h1>
        <p className="text-sm text-gray-400 mt-0.5">Rate your experiences and help the community</p>
      </div>
      ...
      {tab === "pending" && (
        <div className="flex flex-col gap-5">
          ...
          {pending.map((r) => (
            <div key={r.id} className="rounded-2xl border border-gray-100 bg-white shadow-sm p-6">
              <div className="flex items-center gap-3 mb-5 pb-5 border-b border-gray-100">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-400">{r.provider} · {r.date} · ${r.price}</p>
                </div>
              </div>
              <div className="mb-4">
              ...
              <div className="mb-4">
                <p className="text-sm font-semibold text-gray-700 mb-2">Your Review <span className="text-gray-400 font-normal">(optional)</span></p>
                <textarea
                  value={comments[r.id] ?? ""}
            ...
            <div key={r.id} className="rounded-2xl border border-gray-100 bg-white shadow-sm p-6">
              <div className="flex items-start justify-between gap-3 mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-bold text-gray-900">{r.title}</p>
                    <p className="text-xs text-gray-400">{r.provider} · {r.date}</p>
                  </div>
                </div>

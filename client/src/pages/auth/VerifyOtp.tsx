...
import { ArrowUpRight, ShieldCheck, CheckCircle, Clock, RefreshCw, ArrowRight, Mail } from 'lucide-react'
...
const useOtpInput = (length: number = 6) => {
  ...
  const handlePaste = (e: React.ClipboardEvent<HTMLInputElement>) => {
    ...
    setTimeout(() => inputRefs.current[next !== -1 ? next : length - 1]?.focus(), 0)
  }
  ...
  return { values, inputRefs, handleChange, handleInput, handleKeyDown, handlePaste, reset, getOtp, isComplete }
}
...
const extractErrorMessage = (error: unknown, fallback: string): string => {
  if (error && typeof error === 'object' && 'response' in error) {
    ...
    if (r?.status === 429) return 'Please wait 30 seconds before requesting another OTP'
...
const VerifyOtp = () => {
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      setError(msg)
      if ((err as any)?.response?.status === 429) countdown.start(30)
    }
  }
          ...
          <form onSubmit={handleVerify} className="space-y-6">
            ...
            <button
              ...
              className={`flex w-full items-center justify-center gap-2 rounded-xl bg-[#719FC4] py-2.5 text-sm font-bold text-white shadow-sm transition-all duration-200 hover:bg-[#5585A8] hover:shadow-md focus:outline-none focus:ring-2 focus:ring-[#719FC4]/40 ${
                isVerifying || !otp.isComplete() ? 'cursor-not-allowed opacity-60' : 'hover:-translate-y-px'
              }`}
            >
          ...
          <div className="mt-6 text-center">
            ...
            >
              {isResending ? (
                ...
                <>
                  <Clock className="h-3.5 w-3.5" /> Resend in {countdown.formatTime(countdown.countdown)}
                </>
              ) : (
                <>
      ...
      <div className="hidden lg:flex flex-1 flex-col justify-center overflow-hidden bg-[#719FC4] p-16 relative">
        ...
        >
          <h2 className="text-4xl xl:text-5xl font-extrabold tracking-tight text-white leading-tight mb-5">
            One last step<br />to get in.
          </h2>
          <p className="text-white/70 text-base leading-relaxed max-w-sm mb-10">
            We just need to confirm that email address is yours. Enter the code we sent and you're all set.
          </p>

          {/* Info cards */}
          <div className="space-y-3">
            {[
              { icon: Mail,        title: 'Check your inbox',      desc: `Code sent to ${email || 'your email'}` },
              { icon: Clock,       title: 'Code expires in 10 min', desc: 'Request a new one if it expires' },
              { icon: ShieldCheck, title: 'Secure verification',   desc: 'Your account is protected' },
            ].map((item) => (
              <div
                key={item.title}
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-white/55 mt-0.5 truncate max-w-[220px]">{item.desc}</p>
                </div>
              </div>
            ))}
          </div>

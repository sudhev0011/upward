...
import {
  Mail, Lock, User, ArrowRight, ArrowUpRight,
  Eye, EyeOff, Briefcase, ShieldCheck, Zap, Star,
} from "lucide-react";
...
const panelFeatures = [
  ...
  { icon: Zap, title: "Matched in Minutes", desc: "Our algorithm connects you with the right person fast." },
  { icon: Star, title: "Satisfaction Guaranteed", desc: "Pay only when you're 100% happy with the work." },
];
...
const Register = () => {
  ...
  const { mutate: registerMutation, isPending, error: apiError } = useRegisterMutation();
  ...
  return (
    <div className="min-h-screen flex bg-gray-50">
      ...
      <div className="hidden lg:flex flex-1 flex-col justify-between overflow-hidden bg-gray-900 p-16 relative">
        <div className="pointer-events-none absolute inset-0" style={{ backgroundImage: "linear-gradient(rgba(113,159,196,0.07) 1px, transparent 1px), linear-gradient(90deg, rgba(113,159,196,0.07) 1px, transparent 1px)", backgroundSize: "48px 48px" }} />
        ...
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="relative">
          ...
          <p className="text-gray-400 text-base leading-relaxed max-w-sm mb-10">
            Whether you're a client looking for help or a professional ready to grow — Upward is your platform.
          </p>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.4 }} className="relative flex items-center gap-4 rounded-2xl border border-white/8 bg-white/5 p-5">
          <div className="flex -space-x-2">
            {["SK", "JR", "PM", "DC"].map((init, i) => (
              <div key={init} className="flex h-8 w-8 items-center justify-center rounded-full border-2 border-gray-900 bg-[#719FC4] text-[10px] font-bold text-white" style={{ zIndex: 4 - i }}>
                {init}
              </div>
            ))}
          </div>
          <div>
            ...
            <p className="text-xs text-gray-500">already getting work done on Upward</p>
          </div>
        </motion.div>
      </div>
      ...
      <div className="flex flex-1 flex-col justify-center px-6 py-12 sm:px-12 lg:w-1/2 lg:flex-none xl:w-5/12">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45 }} className="mx-auto w-full max-w-sm">
          
          <Link to="/" className="mb-8 inline-flex items-center gap-2 group lg:hidden">
          ...
          <div className="mb-8">
            ...
            <p className="mt-2 text-sm text-gray-500">Join us and start your journey today.</p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative group">
                ...
                <input
                  ...
                  {...register("password")}
                />
                <button type="button" onClick={() => setShowPw(!showPw)} className="absolute inset-y-0 right-0 flex items-center pr-3.5 text-gray-400 hover:text-gray-600">
            ...
            {/* Role selector */}
            <div>
              <label className="mb-2 block text-sm font-semibold text-gray-700">I want to join as a</label>
                ...
                {(["client", "provider"] as const).map((r) => (
                  ...
                  >
                    <input
                      ...
                      {...register("role")}
                    />
                    {r === "client" ? <User className="h-3.5 w-3.5" /> : <Briefcase className="h-3.5 w-3.5" />}
            ...
            {/* API Error Box */}
            {apiError && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="rounded-xl border border-red-100 bg-red-50 p-3 text-xs font-medium text-red-600">
            ...
            <div className="relative my-1">
              ...
              <div className="relative flex justify-center text-xs"><span className="bg-gray-50 px-3 text-gray-400 font-medium">or continue with</span></div>
            </div>

          ...
          <thead>
            <tr className="border-b border-gray-50">
              {[
                ...
                <th
                  key={h}
                  className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-[0.12em] text-gray-400"
                >
                  {h}
                </th>
              ))}
              ...
              >
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-2">
                    <div className="flex h-7 w-7 items-center justify-center rounded-full bg-[#719FC4] text-[10px] font-bold text-white shrink-0">
                      {b.providerInitials}
                    </div>
                ...
                <td className="px-6 py-4">
                  <span
                    className={`rounded-full px-2.5 py-1 text-xs font-semibold ${CATEGORY_STYLES[b.category]}`}
                  >
                    {b.category}
                  </span>
                </td>
                ...
                <td className="px-6 py-4">
                  <span
                    className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold capitalize ${STATUS_STYLES[b.status]}`}
                  >
                    {b.status}
                  </span>
                </td>
            ...
            <div className="flex items-start justify-between gap-2 mb-2">
              <div>
                ...
                <div className="flex items-center gap-1.5 mt-1">
                  <span
                    className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${CATEGORY_STYLES[b.category]}`}
                  >
                    {b.category}
                  </span>
                  <span
                    className={`rounded-full border px-2 py-0.5 text-[11px] font-semibold capitalize ${STATUS_STYLES[b.status]}`}
                  >
                    {b.status}
                  </span>
                </div>
